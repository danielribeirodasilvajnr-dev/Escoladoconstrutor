...
const growthData = [
  { month: 'JAN', value: 30 },
  { month: 'FEB', value: 45 },
  { month: 'MAR', value: 90, active: true },
  { month: 'APR', value: 55 },
  { month: 'MAY', value: 65 },
  { month: 'JUN', value: 80 },
];
    ...
    <div className="p-10 max-w-[1600px] mx-auto space-y-10 pb-20">
      <header className="flex justify-between items-start mb-12">
        ...
        <div className="flex gap-4">
          <button className="p-2.5 bg-[#1a1c22] border border-white/5 rounded-xl hover:bg-[#2a2d35] transition-all text-[#64748b] hover:text-white">
            <Plus className="w-5 h-5" />
          </button>
          <div className="w-12 h-12 rounded-xl bg-[#1a1c22] border border-white/5 p-1">
             <img src="https://i.pravatar.cc/100?u=admin" className="w-full h-full rounded-lg object-cover" alt="Admin" />
          </div>
        </div>
      </header>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={item.label} className="flex justify-between items-center text-sm">
                  ...
                  <span className={cn("font-bold text-white", item.color)}>{item.value}</span>
                </div>
              ))}
            </div>
          </div>
